...
void setTempo(void)
{
	...
	if ((HotSwapID > 0) && (renderMode[5] == 0x00))
	{
...
void runWaterVertex()
{
	...
	GlobalAddressA = (long)(&ok_ScrollTranslucent + (LoopValue * 2) + 1);
	...
	for (int CurrentWater = 0; CurrentWater < LoopValue; CurrentWater++)
	{
		...
		GlobalIntA = *(long*)(GlobalAddressA + (CurrentWater * 8) + 8) & 0xFF;
		MakeWaterVertex(GlobalAddressB,(char)GlobalIntA,0,0,0);
	}
}
...
void loadHotSwap(int gpOffset, int courseValue)
{
	//version 4
	if (HotSwapID > 0)
	{
		...
		*sourceAddress = *(long*)(&ok_HeaderOffsets + ((courseValue + gpOffset) * 1) + ((HotSwapID-1) * 0x14));
		dataLength = 0x6C;
